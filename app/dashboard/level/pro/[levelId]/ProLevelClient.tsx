...
import { PracticeType, PLACEHOLDER_QUESTIONS, AUDIO_PRACTICE_DATA, VIDEO_PRACTICE_DATA } from "@/app/constants/practice";
...
export default function ProLevelClient({ session, levelId }: { session: any, levelId: string }) {
    ...
    const [shuffledOrder, setShuffledOrder] = useState<PracticeType[]>(() => shuffleArray(Object.values(PracticeType)));
    ...
    const getFontSize = (text: string) => {
        ...
        return text.length > 2 ? "text-[11px] md:text-lg font-normal bg-gray-50 p-2 md:p-5 rounded-2xl border border-gray-200 text-center" : "text-3xl md:text-[6rem] font-bold text-center";
    };
    ...
    const absoluteTotalItems = useMemo(() => currentLevelContent.reduce((acc, curr) => acc + (curr.content?.length || 0), 0), [currentLevelContent]);
    const itemsInPreviousSteps = useMemo(() => currentLevelContent.slice(0, step).reduce((acc, curr) => acc + (curr.content?.length || 0), 0), [currentLevelContent, step]);
    const globalCompletedItems = itemsInPreviousSteps + subStep + 1;
    const progressPercentage = absoluteTotalItems > 0 ? (globalCompletedItems / absoluteTotalItems) * 100 : 0;
    const currentItem = currentLevelContent.length > 0 && currentLevelContent[step] ? currentLevelContent[step].content[subStep] : null;
    const isLastItemInLevel = step === currentLevelContent.length - 1 && (subStep + 1) === (currentLevelContent[step]?.content?.length || 0);
    const isPracticeTime = isVocabLevel && currentItem?.itemA && (showPractice || globalCompletedItems % 10 === 0) && !isLastItemInLevel;
    ...
    const randomAudio = useMemo(() => shuffleArray([...AUDIO_PRACTICE_DATA])[0], [isPracticeTime]);
    const randomVideo = useMemo(() => shuffleArray([...VIDEO_PRACTICE_DATA])[0], [isPracticeTime]);
    ...
    if (loading) return <div className="flex h-screen items-center justify-center bg-gray-50 text-gray-500">Syncing Pro Experience...</div>;
