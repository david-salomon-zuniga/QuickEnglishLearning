...
async function updateItemDVerbs() {
    console.log("🚀 Starting bulk update for Level 5 Verbs...");

    const targetLevelId = 5;
    const startTargetItemA = "Abolish, Absorb, Accelerate, Accommodate, Accomplish";
    ...
    try {
        ...
        for (let i = startIndex; i < allContent.length; i++) {
            ...
            const newItemD = `Say to the user to write sentences using these verbs: (${row.item_a}, ${row.item_c})`;

            await supabase.from('level_content').update({ item_d: newItemD }).eq('id', row.id);
        }
        console.log('✨ Level 5 Verbs update complete!');
    } catch (err) {
