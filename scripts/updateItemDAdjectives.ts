...
async function updateItemDAdjectives() {
    console.log("🚀 Starting bulk update for Level 6 Adjectives...");

    const targetLevelId = 6;
    const startTargetItemA = "Absent, Abstract, Absurd, Accessible, Accountable";
    ...
    try {
        ...
        for (let i = startIndex; i < allContent.length; i++) {
            ...
            const newItemD = `Say to the user to write sentences using these adjectives: (${row.item_a}, ${row.item_c})`;

            await supabase.from('level_content').update({ item_d: newItemD }).eq('id', row.id);
        }
        console.log('✨ Level 6 Adjectives update complete!');
    } catch (err) {
        console.error('💀 Failed:', err);
    }
}

updateItemDAdjectives();